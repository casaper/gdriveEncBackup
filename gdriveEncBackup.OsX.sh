...
#####################################################################
TAR_OPTIONS="-pv ${TAR_OPTIONS_OPTIONAL} --${COMPRESSOR} ${NEWER}" 
...
function getMountSpaceFree {
	MOUNT_FREE_1KBLOCKS=$(df $1 | cut -d' ' -f4 | tr '\n' ' ' | cut -d' ' -f2)
	case $2 in
		"" )
...
rm ${BACKUP_DESTINATION}$TAR_FILE_NAME
...
{			
	...
	"GnuPG":
	{
		...
		"Process Time" :
		{
			...
			"End" 	  : "'$(date -r "${GPG_TIME_FINISHED}" +%FT%T%z)'",
...
BACKUP_JSON_RECORD='
{
	"Backup":{
		...
		"Time" :
		{
			...
			"End"     : "'$(date -r "${WHOLE_SCRIPT_TIME_FINISHED}" +%FT%T%z)'",
